...
void MainWindow::on_pushButton_clear_clicked() { ui->value->clear(); }
...
void MainWindow::on_pushButton_equal_clicked() {
  ...
  result = parsing(expr, &stack_num, &stack_op, x);

  QString resultText = QString::number(result, 'f', 7);

  int dotIndex = resultText.indexOf('.');
  if (dotIndex != -1) {
    int endIndex = resultText.length() - 1;
    while (endIndex > dotIndex && resultText[endIndex] == '0') {
      endIndex--;
    }
    if (resultText[endIndex] == '.') {
      endIndex--;
    }
    resultText = resultText.left(endIndex + 1);
  }

  ui->value->setText(resultText);
}
...
void MainWindow::on_pushButton_grapf_clicked() { paintGrath(); }
